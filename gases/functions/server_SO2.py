    ...
    for i in range(difference):
        ...
        for j in filenames:
            try:
                ...
                dataset = ee.ImageCollection('COPERNICUS/S5P/NRTI/L3_SO2') \
                            .filter(ee.Filter.date(fechaI, fechaF))

                col_final = dataset.mean().select('SO2_column_number_density')

                geom = geemap.geojson_to_ee(j)
                geom = geom.select('id_ciud_N', 'SO2_column_number_density')

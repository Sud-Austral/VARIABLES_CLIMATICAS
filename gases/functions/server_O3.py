...
def descarga():
    ...
    dfHistorico = pd.read_excel('gases/functions/descarga/gases_O3.xlsx')
    ...
    for i in range(difference):
        ...
        for j in filenames:
            try:
                ...
                dataset = ee.ImageCollection('COPERNICUS/S5P/NRTI/L3_O3') \
                            .filter(ee.Filter.date(fechaI, fechaF))

                col_final = dataset.mean().select('O3_column_number_density')

                geom = geemap.geojson_to_ee(j)
                geom = geom.select('id_ciud_N', 'O3_column_number_density')
...
def consolidar():
    ...
    if(len(filenamesDelete) > 0):
        ...
        finalDf.to_excel('gases/functions/descarga/gases_O3.xlsx', index=False)

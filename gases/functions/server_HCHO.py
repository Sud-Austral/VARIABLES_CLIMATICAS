    ...
    for i in range(difference):
        ...
        for j in filenames:
            try:
                ...
                dataset = ee.ImageCollection('COPERNICUS/S5P/NRTI/L3_HCHO') \
                            .filter(ee.Filter.date(fechaI, fechaF))

                col_final = dataset.mean().select('tropospheric_HCHO_column_number_density')

                geom = geemap.geojson_to_ee(j)
                geom = geom.select('id_ciud_N', 'tropospheric_HCHO_column_number_density')
